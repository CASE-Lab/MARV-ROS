...
$path/start_scripts/canconfig.sh
sleep 1
...
screen -S ros-marv -p marv-bash -X stuff "source /opt/ros/galactic/setup.bash
source $path/colcon_ws/install/setup.bash
"

# Start all packages
echo "Starting MARV Driver"
screen -S ros-marv -p marv_driver -X stuff "source /opt/ros/galactic/setup.bash
source $path/colcon_ws/install/setup.bash
^l
...
screen -S ros-marv -p marv_scenarios -X stuff "source /opt/ros/galactic/setup.bash
source $path/colcon_ws/install/setup.bash
^l
...
screen -S ros-marv -p sbg_ins -X stuff "source /opt/ros/galactic/setup.bash
source $path/colcon_ws/install/setup.bash
^l
...
screen -S ros-marv -p ros_bag -X stuff "source /opt/ros/galactic/setup.bash
source $path/colcon_ws/install/setup.bash
^l
./start_scripts/start_ros_bag.sh
"
echo "Starting Specific ROS Bag"
screen -S ros-marv -p specific_ros_bag -X stuff "source /opt/ros/galactic/setup.bash
source $path/colcon_ws/install/setup.bash
^l
...
# Prevent service from exiting script
sleep 1