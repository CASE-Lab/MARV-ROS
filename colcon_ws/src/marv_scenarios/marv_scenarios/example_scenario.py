...
class Example_Scenario(Scenario):
    ...
    def __init__(self):
        ...
        __scenario_name = "EXAMPLE"
        # Scenario data variable headers, set to "" if not used, max 7 characters
        __data_var_1_header = "ANGLE"
        __data_var_2_header = "YES"

        # Initialize Scenario Wrapper (handles the logic for starting and stopping scenarios)
        super().__init__('example_scenario',__scenario_name,__data_var_1_header,__data_var_2_header)
    ...
    def main_loop_callback(self):
        ...
        if self.get_scenario_running():
            if self.node_test_timer.elapsed() == 0:
                self.node_test_timer.start()
                self.set_cmd_steering_angle(22.0) # Set steering angle variable
                self.set_progress("1/3") # Update progress variable shown on the OCU
                self.get_logger().info("Main loop started")
                self.get_logger().info("cmd_angle: " + str(self.get_cmd_steering_angle()))

            elif self.node_test_timer.elapsed() > 2 and self.node_test_timer.elapsed() < 4:
                self.set_cmd_steering_angle(-22.0)
                self.set_progress("2/3")
                self.get_logger().info("cmd_angle: " + str(self.get_cmd_steering_angle()))

            elif self.node_test_timer.elapsed() > 6:
                self.set_cmd_steering_angle(0.0)
                self.set_progress("3/3")
                self.get_logger().info("cmd_angle: " + str(self.get_cmd_steering_angle()))

            if self.node_test_timer.elapsed() > 8:
                self.node_test_timer.stop()
                self.get_logger().info("Main loop stopped")
                self.set_state_finished() # Set scenrio state, will be shown on OCU display

            
            self.set_data_var_1(str(round(self.get_cmd_steering_angle(),2))) # Update data variable 1, shown on the OCU
            self.set_data_var_2(":)")
...
def main(args=None):
    ...
    rclpy.spin(example_scenario)

    example_scenario.destroy_node()
    rclpy.shutdown()
