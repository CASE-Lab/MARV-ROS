...
''' 
# ------------------------------------- #
# MARV Test Constant Velocity LQI       #
...
class Test_Constant_Velocity_LQI(Scenario):
    ...
    def __init__(self):
        ...
        self.aps_signal = 0.0
        self.aps_signal_max = 60.0 # Cap aps value for safety

        # Variables for LQI reg
        self.integrator = 0.0
    ...
    def main_loop_callback(self):
        ...
        if self.get_scenario_running():
            ...
            # After 2 seconds start controller
                ...
                # Update LQI intergrator
                self.integrator += self.error
                # Update aps signal
                self.aps_signal = self.current_velocity*self.Kp + self.integrator*self.Ki

                # Cap signal within reasonable limits (Saturation)
            ...
            self.get_logger().info("VEL: " + str(round(self.current_velocity,2)))
...
def main(args=None):
    ...
    test_constant_velocity_lqi = Test_Constant_Velocity_LQI()

    rclpy.spin(test_constant_velocity_lqi)

    test_constant_velocity_lqi.destroy_node()
    rclpy.shutdown()
